...
export default function IndexMainMenu(props: IndexMainMenuProps) {
  ...
  const allItems: MenuItem[] = useMemo(() => [
   ...
   { emoji: "⏱️", label: isHe ? "משחק הזמנים" : "Tense Switch", hub: "grammar", level: "intermediate", action: () => { window.location.hash = "#/tense-switch"; } },
    // Foundations
    ...
    { emoji: "🎯", label: isHe ? "רוצה · צריך · יכול · אוהב" : "Want/Need/Can/Love", hub: "foundations", level: "beginner", action: () => setShowModalVerbs(true) },
      ...
      <div className="max-w-2xl mx-auto px-4 py-3">
        {showHubGrid ? (
          <>
            ...
            {/* Featured: Active & Passive */}
            <button
              onClick={() => { window.location.hash = "#/active-passive"; }}
              style={{ opacity: cardsVisible ? 1 : 0, transform: cardsVisible ? "translateY(0)" : "translateY(20px)", transition: `opacity 0.4s ease 60ms, transform 0.4s ease 60ms` }}
              className="group w-full mb-4 p-5 rounded-2xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 text-white shadow-md hover:shadow-lg active:scale-[0.98] transition-all text-right"
            >
              <div className="flex items-center gap-4">
                <span className="text-5xl group-hover:scale-110 transition-transform duration-200 shrink-0">⚡</span>
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-lg leading-tight">פעיל וסביל</div>
                  <div className="text-sm text-white/90 mt-1">למדו 6 בניינים דרך סיפור, טבלה ותרגול אינטראקטיבי</div>
                </div>
                <span className="text-2xl opacity-80 group-hover:opacity-100 transition-opacity">➡️</span>
              </div>
            </button>
            {/* Featured: Jewish Holidays */}
            <button
              onClick={() => { window.location.hash = "#/jewish-holidays"; }}
              style={{ opacity: cardsVisible ? 1 : 0, transform: cardsVisible ? "translateY(0)" : "translateY(20px)", transition: `opacity 0.4s ease 90ms, transform 0.4s ease 90ms` }}
              className="group w-full mb-4 p-5 rounded-2xl bg-gradient-to-br from-amber-400 via-rose-400 to-blue-500 text-white shadow-md hover:shadow-lg active:scale-[0.98] transition-all text-right"
            >
              <div className="flex items-center gap-4">
                <span className="text-5xl group-hover:scale-110 transition-transform duration-200 shrink-0">🗓️</span>
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-lg leading-tight">חגי ישראל</div>
                  <div className="text-sm text-white/90 mt-1">8 חגים, סיפורים, ברכות ותרגול אוצר מילים</div>
                </div>
                <span className="text-2xl opacity-80 group-hover:opacity-100 transition-opacity">➡️</span>
              </div>
            </button>
            ...
            {/* Featured: Adjectives Drill */}
            <button
              onClick={() => { window.location.hash = "#/adjectives-drill"; }}
              style={{ opacity: cardsVisible ? 1 : 0, transform: cardsVisible ? "translateY(0)" : "translateY(20px)", transition: `opacity 0.4s ease 270ms, transform 0.4s ease 270ms` }}
              className="group w-full mb-4 p-5 rounded-2xl bg-gradient-to-br from-amber-400 via-orange-500 to-rose-500 text-white shadow-md hover:shadow-lg active:scale-[0.98] transition-all text-right"
            >
              <div className="flex items-center gap-4">
                <span className="text-5xl group-hover:scale-110 transition-transform duration-200 shrink-0">🎭</span>
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-lg leading-tight">תרגול שמות תואר</div>
                  <div className="text-sm text-white/90 mt-1">50 זוגות ניגודים, הטיה למגדר ומספר ומשפטים להשלמה</div>
                </div>
                <span className="text-2xl opacity-80 group-hover:opacity-100 transition-opacity">➡️</span>
              </div>
            </button>
